...
class SelfPlayWrapper(BombermanEnv):
    ...
    def step(self, action):
        ...
        for agent in self._env.agents:
            if agent != self.controlled:
                obs, rew, term, trunc, _ = self._env.last()
                if term or trunc:
                    a = None
                    ...
                    a = self._env.action_space(agent).sample()
                self._env.step(a)
        obs, rew, term, trunc, info = self._env.last()
        return self._flatten(obs), rew, term, trunc, info